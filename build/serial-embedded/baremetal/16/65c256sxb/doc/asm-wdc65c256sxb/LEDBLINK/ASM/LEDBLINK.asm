...
;***********************************************************************
;RTC TOD routine VECTOR in SP RAM - blink LED every second
;THIS IS PERSISTENT DURING RESET !!!

;HERE it is WEIRD that SOMETIMES, uploading BIN doesnot update such area?
;isnt internal/external SRAM ZP+SP switching by mismatch sometimes ???
rtc
    .section offset $01c0
    .org $01c0
    jmp rtc_tod ;NO, its not possible to skip from RTC TOD routine to external SRAM !!!
    .ends


;***************************************************************************
;                    W65C265S_BLINK Code Section
;***************************************************************************
...
START:
    ...
    sei
    cld
    ; ldx	#$ff ;isnt this weird ??? as it is called by alread initialized monitor with running IRQs ???
    ; txs
    ...
    lda #$FB
    sta PCS7 ;P72 data direction as OUT

blink:

    ;output decimal seconds (ok up to 99)
    lda blink_counter+1
    ; jsl DEC
      ; jsl SEND_HEX
      ; lda blink_counter
...
;!! NOT POSSIBLE to place RTC TOD routine here, this is external SRAM !!!
rtc_tod:
    lda #$FB
    sta PCS7 ;P72 data direction as OUT
    jsr delay
    ...
    lda #$04        ;OFF
    sta PD7
    rtl ;in fact JSR here required by monitor ROM low power routine, but we try to call it by JSL from console
...
;***********************************************************************
; LOW POWER RTC TOD SIGNATURE - CLEARED AFTER RESET !!!
lowpwr  .section offset $dfb3
    .db $55 ;RTC TOD interrupt at low power
    .db $AA ;RTC TOD interrupt at low power
    .ends ;lowpwr

;DFC0 - interesting !!! writing byte to this address defines entire DFC0-DFFF !!!
;                       after RESET, value is returned to 10 !!!
